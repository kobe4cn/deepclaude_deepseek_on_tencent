...
//deepseek 原声api地址 :"https://api.deepseek.com/chat/completions";
//默认模型： deepseek-reasoner
// pub(crate) const DEEPSEEK_API_URL: &str = "https://api.lkeap.cloud.tencent.com/v1/chat/completions";
pub(crate) const DEEPSEEK_API_URL: &str = "https://api.lkeap.cloud.tencent.com/v1/chat/completions";
...
pub struct StreamResponse {
    ...
    //非deepseek官方的api返回中不包含这个，需要通过option设置或者default设置避免serde json序列化出错
...
impl Default for CompletionTokensDetails {
    fn default() -> Self {
        Self {
            reasoning_tokens: 0,
        }
        ...
        Box::pin(async_stream::try_stream! {
            ...
            let mut stream = request_builder
                ...
                .map_err(|e| ApiError::DeepSeekError {
                    ...
                    message: format!("concurrency exceeded: {}", e),
                    type_: "runtime_error".to_string(),
                    param: Some("null".to_string()),
                    code: Some("20034".to_string())
                })?
                .bytes_stream();
...
async fn test_deepseek_chat() {
    ...
    let request_value = serde_json::json!({
        "messages": vec![Message {
            role: Role::User,
            content: "Rust是一门什么样的语言，应该如何更好的学习掌握，规划合理的学习路径".to_string(),
        }],
        "stream": false,
        // Set defaults only if not provided in config
        "model": "deepseek-r1",
